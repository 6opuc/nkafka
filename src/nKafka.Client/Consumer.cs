...
public class Consumer<TMessage> : IConsumer<TMessage>
{
    ...
    {
        using var joinGroupActivity = KafkaTracing.Source.StartActivity("Consumer.JoinGroup", ActivityKind.Client);

        using var bootstrapActivity = KafkaTracing.Source.StartActivity("Consumer.BootstrapConnection", ActivityKind.Client);
        await using var bootstrapConnection = await OpenBootstrapConnectionAsync(cancellationToken);
        bootstrapActivity?.AddTag("nKafka.phase", "bootstrap");

        using var coordinatorConnActivity = KafkaTracing.Source.StartActivity("Consumer.OpenCoordinatorConnection", ActivityKind.Client);
        _coordinatorConnection = await OpenCoordinatorConnectionAsync(bootstrapConnection, cancellationToken);
        coordinatorConnActivity?.AddTag("nKafka.phase", "coordinator_connection");

        using var refreshMetadataActivity = KafkaTracing.Source.StartActivity("Consumer.RefreshMetadata", ActivityKind.Client);
        await RefreshMetadataAsync(cancellationToken);
        refreshMetadataActivity?.AddTag("nKafka.phase", "refresh_metadata");

        using var joinGroupReqActivity = KafkaTracing.Source.StartActivity("Consumer.JoinGroupRequest", ActivityKind.Client);
        using var joinGroupResponse = await JoinGroupRequestAsync(_coordinatorConnection, cancellationToken);
        joinGroupReqActivity?.AddTag("nKafka.phase", "join_group_request");
        ...
        using var syncGroupActivity = KafkaTracing.Source.StartActivity("Consumer.SyncGroup", ActivityKind.Client);
        await SyncGroup(_coordinatorConnection, assignments, cancellationToken);
        syncGroupActivity?.AddTag("nKafka.phase", "sync_group");
    ...
    {
        using var refreshCoordinatorActivity = KafkaTracing.Source.StartActivity("Consumer.RefreshCoordinator", ActivityKind.Client);
        var coordinator = await RefreshCoordinatorAsync(cancellationToken);
        refreshCoordinatorActivity?.AddTag("nKafka.phase", "refresh_coordinator");

        using var requestMetadataActivity = KafkaTracing.Source.StartActivity("Consumer.RequestMetadata", ActivityKind.Client);
        using var metadataResponse = await RequestMetadata(coordinator, cancellationToken);
        requestMetadataActivity?.AddTag("nKafka.phase", "request_metadata");
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var findCoordinatorActivity = KafkaTracing.Source.StartActivity("Consumer.FindCoordinator", ActivityKind.Client);
        ...
        findCoordinatorActivity?.AddTag("nKafka.phase", "find_coordinator");
            ...
            async () =>
            {
                while (!cancellationToken.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        heartbeatActivity?.AddTag("nKafka.phase", "heartbeat");
        ...
        using (_logger.BeginScope(context.ToString()))
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    fetchActivity?.AddTag("nKafka.phase", "fetch");
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var deserializeActivity = KafkaTracing.Source.StartActivity("Consumer.Deserialize", ActivityKind.Client);
    ...
    public async ValueTask<IConsumerBatch<TMessage>> ConsumeBatchAsync(CancellationToken cancellationToken)
    {
        ...
        await EnsureEnumeratorAsync(cancellationToken);
        fetchWaitActivity?.AddTag("nKafka.phase", "fetch_wait");
        consumeBatchActivity?.AddTag("nKafka.phase", "consume_batch");
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        using var shutdownActivity = KafkaTracing.Source.StartActivity("Consumer.Shutdown", ActivityKind.Client);

        using var stopFetchingActivity = KafkaTracing.Source.StartActivity("Consumer.StopFetching", ActivityKind.Client);
        await StopFetchingAsync();
        stopFetchingActivity?.AddTag("nKafka.phase", "stop_fetching");
    ...
    private async ValueTask LeaveGroupAsync(CancellationToken cancellationToken)
    {
        ...
        using var leaveGroupActivity = KafkaTracing.Source.StartActivity("Consumer.LeaveGroup", ActivityKind.Client);
        ...
        leaveGroupActivity?.AddTag("nKafka.phase", "leave_group_request");
